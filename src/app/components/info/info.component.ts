...
const DATA : Wymaganie[] = [
  ...
  {wymaganie: "Serwis musi zawierać stronę poświęconą opisowi samego serwisu – jakich technologii użyto, jakie były założenia przy planowaniu nawigacji, oraz dlaczego autor uważa, że strona spełnia wymagania wymienione w tym dokumencie.",
    uzasadnienie: "Stoworzona została strona z informacjami (która jest aktualnie wyświetlana) o użytych technologiach i komentarzem do każdego z wymagań. Co do nawigacji strony, to w celu zapewnienia prostoty używania systemy zdecydowano się na model single-page application, dzięki czemu wszystkie oferowane funkcje są widoczne i łatwo dostępne. Interakcja z użytwkonikiem zazwyczaj odbywa się za pomocą okien dialogowych."},
